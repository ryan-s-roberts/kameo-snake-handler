...
setup_subprocess_system! {
    ...
    parent_init = {
        ...
        runtime.block_on(async {
            ...
            run_sync_tests(python_path.clone()).await?;
            run_async_tests(python_path.clone()).await?;
            run_invalid_config_tests(python_path.clone()).await?;
...
async fn run_sync_tests(python_path: std::path::PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    info!("==== SYNC FLOW ====");
...
async fn run_async_tests(python_path: std::path::PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    info!("==== ASYNC FLOW ====");
...
async fn run_invalid_config_tests(python_path: std::path::PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let invalid_path_config = PythonConfig {
        python_path: vec!["/invalid/path".to_string()],
        module_name: "ork_logic".to_string(),
        function_name: "handle_message".to_string(),
        env_vars: vec![],
        is_async: false,
